...
\par\smallskip
\begin{minipage}{0.96\linewidth}\footnotesize\textit{Note.} Metrics are averaged over model-dataset conditions, with the best probe within each reported signal family selected by validation AURC in the earlier audit and by family averages in the Gemma/Llama run. The conclusion is unchanged: hidden augmentation slightly improves some operating points but does not change the practical rule.\end{minipage}
\end{table}
